...
WEEKLY_ORD_HRS = 38                 # weekly ordinary hours stay the same
CYCLE_LENGTH_WEEKS = 2    
Max_Ord_Hrs_Day = 10
# Define Max Ordinary Hours per Day
#Max_Ord_Hrs_Day = pd.Timedelta(hours=10)
Max_Ord_Hrs = WEEKLY_ORD_HRS * CYCLE_LENGTH_WEEKS  # 76 hours per fortnight
First_2_Hrs_OT_Cutoff = Max_Ord_Hrs + 2        # 78 hours
...
# Addition of cycle length parameter to allow for future changes

timesheet_df['Cycle_Index'] = (weeks_since_anchor // CYCLE_LENGTH_WEEKS).astype(int)        # 0,1,2,...
timesheet_df['Cycle_Week_Index'] = (weeks_since_anchor % CYCLE_LENGTH_WEEKS) + 1           # 1..8




# Commented out for future use of cycle length parameter 14/01/2026
...
timesheet_df['Roster Starting'] = (
    CYCLE_ANCHOR + pd.to_timedelta(timesheet_df['Cycle_Index'] * CYCLE_LENGTH_WEEKS * 7, unit='D')
)
timesheet_df['Roster Ending'] = timesheet_df['Roster Starting'] + pd.Timedelta(days=CYCLE_LENGTH_WEEKS*7 - 1)

# Per-row week ending (Sunday of that week)
timesheet_df['Week Ending'] = timesheet_df['Roster Starting'] + pd.to_timedelta(timesheet_df['Cycle_Week_Index']*7 - 1, unit='D')

# Pay date: Wednesday after Week Ending, with your Tuesday exceptions preserved
wed_offset = (2 - timesheet_df['Week Ending'].dt.weekday + 7) % 7
timesheet_df['Estimated Pay Date'] = timesheet_df['Week Ending'] + pd.to_timedelta(wed_offset, unit='D')
exceptions = [pd.Timestamp('2024-12-25'), pd.Timestamp('2025-01-01')]
...
# Commented out 14/01/2026 to make week number based on cycle week index instead of days before roster end
...
timesheet_df['EmpID_Week_Key'] = (
   ...
   # timesheet_df['Week Number'].astype(str)
    timesheet_df['Cycle_Week_Index'].astype(str)
...
timesheet_df_weekly_for_Leave['EmpID_Week_Key'] = (
    ...
    timesheet_df_weekly_for_Leave['Cycle_Week_Index'].astype(str)
)
...
agg_dict = {
    ...
    'Cycle_Week_Index'       : first_nonnull,
...
def calculate_effective_hours(df):
    ...
    # Identify last week of the roster cycle (e.g., 8 for an 8‑week cycle)
    last_week = CYCLE_LENGTH_WEEKS

    # --- Step 1: Determine Effective Leave ---
    # Rule:
    #   • Exclude leave entirely in the LAST week of the roster cycle (not Week 2)
    #   • Also exclude leave when (worked hours + leave hours) > 50
    df['Effective_Leave'] = np.where(
        df['Cycle_Week_Index'] == last_week,       # last week → exclude all leave
        0,
        np.where(
            (df['Total Leave Hours'] + df['Total TS Hours (OT Adj)']) > 50,
            0,
            df['Total Leave Hours']                # otherwise include leave
        )
    )

    # --- Step 2: Effective total = worked hours + allowable leave ---
    df['Effective_Total'] = df['Total TS Hours (OT Adj)'] + df['Effective_Leave']
...
#     group['Fortnight_Total'] = np.nan
#     #group.loc[group['Week Number'] == 2, 'Fortnight_Total'] = fortnight_total
#     # Still needs to be alte
#     group.loc[group['Cycle_Week_Index'] == 2, 'Fortnight_Total'] = fortnight_total
...
#     # Step 4: Calculate overtime per week
#     #group = group.sort_values('Week Number')  # Ensure weeks are in order
#     group = group.sort_values('Cycle_Week_Index')  # Ensure weeks are in order
#     ot = []
#     for _, row in group.iterrows():
#         # Always exclude leave from Week 2
#        # if row['Week Number'] == 2:
#         if row['Cycle_Week_Index'] == 2:
...
def calculate_overtime(group):
    """
    Calculates overtime for an arbitrary roster cycle length (e.g., 8 weeks).
    Excludes leave from the *last* week of the cycle, since leave-only periods
    shouldn’t drive overtime.
    """

    # --- 1) Total cycle hours ---
    cycle_total = group['Effective_Total'].sum()

    # --- 2) Show total only on the *last week* of the cycle ---
    group['Cycle_Total'] = np.nan
    last_week = CYCLE_LENGTH_WEEKS
    group.loc[group['Cycle_Week_Index'] == last_week, 'Cycle_Total'] = cycle_total

    # --- 3) Excess over Max Ordinary Hours (e.g., 304 for 8 weeks) ---
    excess = max(cycle_total - Max_Ord_Hrs, 0)

    # --- 4) Allocate OT week-by-week in chronological order ---
    group = group.sort_values('Cycle_Week_Index')
    ot = []

    for _, row in group.iterrows():

        # Exclude leave from the *last week* of the cycle
        if row['Cycle_Week_Index'] == last_week:
            effective_hours = row['Total TS Hours (OT Adj)']
        else:
            effective_hours = row['Effective_Total']

        # Allocate OT from remaining excess
        ot_hours = min(effective_hours, excess)
        excess -= ot_hours
        ot.append(ot_hours)

    # --- 5) Cosmetic: show total OT only on first row of the final week ---
    group['Weekly_Overtime_Hours (Incl Leave)'] = np.nan
    idx = group[group['Cycle_Week_Index'] == last_week].index
    if len(idx) > 0:
        group.loc[idx[0], 'Weekly_Overtime_Hours (Incl Leave)'] = sum(ot)
